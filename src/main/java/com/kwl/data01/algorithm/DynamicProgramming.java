...
public class DynamicProgramming {
     ...
     * DP的三要素: 1)初始状态 2)状态转移方程式 3)终止状态
     ...
     * 1 传统的递归
     */
    public int jiechen(int n) throws Exception {
     ...
     */
    public int jieMemory01(int n) throws Exception {
     ...
     */
    public int jiechen01(int n) throws Exception {
        if (n < 0) throw new Exception("负数不存在阶乘,请重新输入!");          //如果n是负数,就抛出异常
        int[] memory = new int[n + 1];        //储存记录功能
    ...
    public int MemoryTopToBottom(int n, int[] memory) throws Exception {
        if (memory[n] != -1) return memory[n];        //如果存在,就直接返回,不需要计算
        if (n == 0 || n == 1) memory[n] = 1;         //递归结束的条件
        else memory[n] = n * MemoryTopToBottom(n - 1, memory);
        return memory[n];
    }
}
